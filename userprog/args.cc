...
SaveArgs(int address) {
    ...
    // The maximum number of arguments was reached but the last is not null.
...
WriteArgs(char **args) {
    ...
    // Save the address of the j-th argument counting from the end down to the beginning.
    for (unsigned j = 0; j < i; j++) machine->WriteMem(sp + 4 * j, 4, args_address[j]);
